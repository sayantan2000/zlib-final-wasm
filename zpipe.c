...
int main(int argc, char **argv)
{
    ...
    if (argc == 2) {
        ...
        int outputFileLength = getFileSize(output);                                  // Getting output file Size in bytes.
        char *a_output = malloc(outputFileLength);                                   // Setting a variable to copy the file contents
        fread(a_output, 1, outputFileLength, output);                                // Copying the file contents into the character array buffer.
        EM_ASM_({window.download($0, $1, $2)}, argv[1], a_output, outputFileLength); // Downloading the file
